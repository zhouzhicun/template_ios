...
DESede_Encrypt(char *to, char *from, unsigned int len, char *key)
{
    ...
    /* encrypt */
    SetKey((unsigned char*)key, EN0);
    for ( p=(unsigned char*)data, q=(unsigned char*)to; p<(unsigned char *)data+len; p+=BLOCK_BYTES, q+=BLOCK_BYTES ) {
        DoCrypt(p,q);
    }
...
{
    unsigned char *p, *q;
    SetKey((unsigned char*)key, DE1);
    for ( p=(unsigned char*)from, q=(unsigned char*)to; p<(unsigned char *)from+len; p+=BLOCK_BYTES, q+=BLOCK_BYTES ) {
        DoCrypt(p, q);
    }
